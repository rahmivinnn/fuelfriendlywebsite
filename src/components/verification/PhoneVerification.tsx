...
import { Phone, Check, AlertCircle, RefreshCw, Clock, Send } from 'lucide-react';
...
interface PhoneVerificationProps {
  onVerificationComplete: (verified: boolean, phoneData?: any) => void;
}
...
const PhoneVerification: React.FC<PhoneVerificationProps> = ({ 
  ...
  const sendVerificationCode = () => {
    ...
    toast({
      ...
      description: `A verification code has been sent to ${countryCode} ${formatPhoneNumber(phoneNumber)}.`,
    });

    // In a real app, this would call an API to send an SMS
    console.log(`Sending verification code ${code} to ${countryCode}${phoneNumber}`);
  };
  ...
  const verifyCode = () => {
    ...
    setTimeout(() => {
      if (verificationCode === generatedCode) {
        ...
        // Pass the verified phone data back to parent
  ...
  const resendVerificationCode = () => {
    ...
    toast({
      title: "New Code Sent",
      description: `A new verification code has been sent to ${countryCode} ${formatPhoneNumber(phoneNumber)}.`,
    });

    // In a real app, this would call an API to send an SMS
    console.log(`Resending verification code ${code} to ${countryCode}${phoneNumber}`);
  };
  ...
  useEffect(() => {
    if (generatedCode) {
      console.log(`DEMO: Your verification code is ${generatedCode}`);
        ...
        <div className="space-y-4">
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-700">
              We've sent a 6-digit verification code to <span className="font-medium">{countryCode} {formatPhoneNumber(phoneNumber)}</span>
            </p>
          </div>
          ...
          <p className="text-xs text-center text-gray-500">
            Didn't receive the code? Check your phone for SMS or try resending after the timer expires.
          </p>
        </div>
      )}
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-start">
            ...
            <div>
              ...
              <p className="text-sm text-green-700">
                Your phone number {countryCode} {formatPhoneNumber(phoneNumber)} has been successfully verified.
              </p>
            </div>
          </div>
        </motion.div>
